...
#import "RecipientChatViewController.h"
#import <QuartzCore/QuartzCore.h>
#import <UIKit/UIKit.h>
#import "GameState.h"

@interface RecipientChatViewController (){
...
@implementation RecipientChatViewController
...
- (void)viewDidAppear:(BOOL)animated{
    ...
    self.tabBarController.title = @"Chat with your Recipient";
...
- (void)loadChatWithLog:(NSArray *)log{
    ...
    for(int x = 0; x < [log count]; x ++){
        ...
        if([title isEqualToString:@"You"]){
            ...
            [self addChatBubbleForYourRecipient:message withName:title];
        }
    }
...
- (void)addChatBubbleForYourRecipient:(NSString *)bubbleText withName:(NSString *)name{
...
- (void)refresh:(NSObject *)obj{
    ...
    isRequesting = YES;
    NSString *sendString = [NSString stringWithFormat:@"http://hwsanta.com/app/home/chatrecipient.php?userid=%@", [[GameState sharedGameState] getUserID]];
...
- (void) alertView:(UIAlertView *)alert clickedButtonAtIndex:(NSInteger)buttonIndex
{
    if(buttonIndex==0){
        ...
        NSString *sendString = [NSString stringWithFormat:@"http://hwsanta.com/app/home/messagerecipient.php?userid=%@&message=%@", [[GameState sharedGameState] getUserID], message ];
        NSURLRequest *request = [[NSURLRequest alloc] initWithURL:[NSURL URLWithString:[sendString stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding]]];
